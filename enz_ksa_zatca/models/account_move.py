...
class AccountMove(models.Model):
    ...
    zatca_onboarding_status = fields.Boolean(readonly=1, default=lambda self: self.env['zatca.config'].search(
        [('company_id', '=', self.company_id.id)]).onboarding_status, copy=False)
    ...
    def create_xml_file(self, previous_hash=0):
        ...
        details = self.env['zatca.config'].search([('company_id', '=', self.company_id.id)])
        ...
        zatca_certificate_status = details.certificate_status
        if zatca_certificate_status != True:
            raise exceptions.MissingError("Register Certificate before proceeding.")

        f = open('/tmp/enz_zatca_'+ company +'_cert.pem', 'r')
        certificate = f.read()
        ...
        if is_tax_invoice:
            if details.invoice_type not in ['1100', '1000']:
        ...
        if not is_tax_invoice and details.invoice_type not in ['1100', '0100']:
            raise exceptions.AccessDenied("Certificate not allowed for Standard Invoices.")
        ...
        ksa_16 = int(details.xml_sequence)
        ksa_16 += 1
        details.xml_sequence = str(ksa_16)
        ...
        details.pih = self.zatca_invoice_hash
        if signature:
            ...
            f.close()
            signature = '''openssl dgst -sha256 -sign /tmp/enz_zatca_'''+ company +'''_privatekey.pem /tmp/''' + hash_filename + ''' | base64 /dev/stdin'''
            print(signature)
    ...
    def compliance_invoices_api(self):
        ...
        if details.status == 'Onboarding failed, restart process !!':
            raise exceptions.AccessDenied('Onboarding failed, restart process !!')

        zatca_on_board_status_details = json.loads(details.on_board_status_details)
        ...
        user = details.sb_bs_token
        password = details.sb_secret
        ...
        try:
            ...
            if req.status_code == 500:
                ...
                if response['validationResults']['errorMessages'] == [] and response['validationResults']['status'] == 'PASS' and \
                    ...
                    details.on_board_status_details = json.dumps(zatca_on_board_status_details)
                    total_required = []
                    ...
                    if invoices_required == '0':
                        details.status = "Onboarding completed, request for production credentials now"
                        details.otp = None
                        details.onboarding_status = 1
                        string += "<tr><td colspan='6'><center><b>" + \
                                  str("Onboarding completed, request for production credentials now") + "</b></center></td></tr>"
                    else:
                        on_board_status = json.loads(details.on_board_status_details)
                        status = ''
                        ...
                        if on_board_status.get('simplified', 0):
                            ...
                            if not on_board_status.get('simplified', 0).get('debit', 0):
                                status += "debit,"
                        zatca_status = details.status
                        zatca_status[:zatca_status.find('invoices\n') + 9] + status
                        details.status = zatca_status[:zatca_status.find('invoices\n') + 9] + status

                        details.status = details.status[:29] + invoices_required + details.status[30:]
                        string += "<tr><td colspan='6'><center><b>" + \
                    ...
                    string += "</table>"
                    details.on_board_status_details = json.dumps(zatca_on_board_status_details)
                    details.status = 'Onboarding failed, restart process !!'
                    details.onboarding_status = 0
            ...
            return {
                ...
                'view_mode': 'form',
                'res_id': self.id,
                'views': [(self.env.ref('enz_ksa_zatca.zatca_response').id, 'form')],
            }
    ...
    def compute_qr_code_str(self, signature_value, is_tax_invoice, bt_112, bt_110):
        ...
        for record in self:
            ...
            if record.l10n_sa_confirmation_datetime and record.company_id.vat:
                ...
                f = open('/tmp/enz_zatca_'+ company +'_cert_publickey.bin', 'rb')
                cert_pub_key = f.read()
                ecdsa_public_key = get_qr_encoding(8, cert_pub_key)
                if not is_tax_invoice:
                    ecdsa_cert_value = get_qr_encoding(9, binascii.unhexlify(details.cert_sig_algo))
            ...
            temp = BytesIO()
            qrcode.make(qr_code_str).save(temp, format="PNG")
            self.qr_img = base64.b64encode(temp.getvalue())
