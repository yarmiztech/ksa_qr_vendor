...
class ResPartner(models.Model):
    ...
    # bt_46 (BR-KSA-14)
    buyer_identification_no = fields.Char(string="Buyer Identification Number (Other buyer ID)", required=1)
    ...
    def set_default_values_to_fields(self):
        ...
        partners_list = self.env['res.company'].search([]).mapped("partner_id.id")
        for partner in partners:
            if partner.id not in partners_list:
                if partner.customer_rank > 0:
                    if partner.vat:
                        print(len(partner.vat))
                        if len(partner.vat) != 15:
                            partner.vat = 323412341234123
                    partner.arabic_name = "."
                    partner.building_no = "3799"
                    partner.additional_no = "2121"
                    partner.buyer_identification = 'TIN'
                    partner.buyer_identification_no = '123457890'
                    if not partner.street:
                        partner.street = "King Abdullah Road"
                    if not partner.district:
                        partner.district = "Riyadh"
                    if not partner.city:
                        partner.city = "Riyadh"
                    if not partner.zip:
                        partner.zip = "11564"
